...
function displayOverUnderDecimal(balance: string) {
  if (9999999 < Number(balance)) return <Image src={OverAmount} alt={'OverAmount'} width={13} height={13} />;
  else if (0.0001 > Number(balance)) return <Image src={UnderAmount} alt={'underAmount'} width={13} height={13} />;
  else return;
}

function formatSymbol(symbol: string) {
  if (symbol.length > 7) return symbol.slice(0, 5) + '...';
  else return symbol;
}
